...
garden_harvest %>% 
  ...
  arrange(vegetable, days_to_first_harvest)
```
...
```{r}
garden_harvest %>% 
  semi_join(supply_costs %>% filter(brand == "Renee's Garden"),
            by = c("vegetable", "variety"))
```
...
garden_harvest %>% 
  filter(vegetable == "tomatoes") %>% 
  count(variety = fct_relevel(variety, "Black Krim", "Cherokee Purple"))
```
...
garden_harvest %>% 
  filter(vegetable == "tomatoes") %>% 
  ggplot(aes(y = fct_relevel(variety, "Black Krim", "Cherokee Purple"))) +
  geom_bar() +
  labs(y = "", title = "# of harvests by tomato variety")
```
...
garden_harvest %>% 
  filter(vegetable == "tomatoes") %>% 
  mutate(variety2 = fct_recode(variety,
                               Grape = "grape",
...
```{r}
garden_harvest %>% 
  mutate(vegetable_grp = fct_lump(vegetable,
                                  n = 9, 
                                  other_level = "less_used_veggies")) %>% 
  count(vegetable_grp) %>% 
  arrange(desc(n))
```
...
```{r}
family %>% 
  mutate(first_three = str_sub(name, 1, 3),
         last_three = str_sub(name, -3,-1))
```
...
```{r}
family %>% 
  mutate(has_is = str_detect(name, "ad"))
```
...
```{r}
family %>% 
  mutate(has_is = str_detect(name, "A|ad"))
```
